...
<img src="images/Rlogo" alt="" width="100px" />
...
- Later we will focus into `read_tier()` function
- After that individual parsing method can be baked into a new function
...
corpus <- dir('../adv_elan_draft/corpus', pattern = 'eaf$', full.names = TRUE) %>%
  map(FRelan::read_eaf) %>%
  bind_rows() %>%
...
```{r}
metadata <- dir('../adv_elan_draft/corpus', 'cmdi$', full.names = TRUE) %>%
